...
export default function CurrentWeather({ data, selectedCity }: CurrentWeatherProps) {
    return (
        <View className='items-center'>
            <Text className='text-white text-xl font-poppins-regular mt-4'>
                Now in{' '}
                <Text className='font-poppins-bold'>{selectedCity}</Text>
            </Text>

            <View className='flex-row px-4 items-center'>
                <View className='items-center'>
                    <View className='flex-row'>
                        <Text className='text-white text-[100px] leading-tight font-poppins-bold'>
                            {Math.round(data?.current.temp_c ?? 0)}
                        </Text>
                        <Text className='text-white text-3xl font-poppins-medium mt-3'>°C</Text>
                    </View>
